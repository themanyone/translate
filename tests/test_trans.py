...
class TestFindRouterModel(unittest.TestCase):
    ...
    def test_matches_translate_alias_when_id_differs(self):
        with mock.patch("translate.urllib.request.urlopen") as up:
            up.return_value = self._models_ok([
                {"id": "some-renamed-model", "aliases": ["translate"]},
            ])
...
class TestResolveBackend(unittest.TestCase):
    ...
    def test_prefers_running_router_with_translategemma(self):
        ...
        with mock.patch(
                "translate.find_router_model", return_value=model_id,
    ...
    def test_fallback_uses_host_port_overrides(self):
        with mock.patch("translate.find_router_model", return_value=None), \
                mock.patch(
                    "translate.ensure_server", return_value="http://127.0.0.1:9000"
                ) as ensure:
            resolve_backend(
...
class TestSpeak(unittest.TestCase):
    def test_piper_plays_directly_without_output_file(self):
        ...
        with mock.patch(
            "translate.voice_for_language", return_value="/fake/ru_RU-x.onnx"
    ...
    def test_returns_false_without_voice(self):
        with mock.patch("translate.voice_for_language", return_value=None), \
...
class TestServerManager(unittest.TestCase):
    ...
    def test_ensure_server_starts_detached_when_unhealthy(self):
        with (
            ...
            mock.patch("translate.subprocess.Popen") as popen,
            mock.patch("translate.wait_for_server", return_value=True) as wait,
            mock.patch("translate.STATE_DIR", Path("/tmp/fake-state")),
        ):
            up.side_effect = URLError("refused")
    ...
    def test_ensure_server_raises_when_start_times_out(self):
        with (
            mock.patch("translate.urllib.request.urlopen") as up,
            mock.patch("translate.subprocess.Popen") as popen,
            mock.patch("translate.wait_for_server", return_value=False),
            mock.patch("translate.STATE_DIR", Path("/tmp/fake-state")),
        ):
            up.side_effect = URLError("refused")
    ...
    def test_stop_server_unlinks_stale_pidfile(self):
        ...
        with tempfile.TemporaryDirectory() as state:
            ...
            with mock.patch("translate.os.kill", side_effect=ProcessLookupError):
    ...
    def test_stop_server_unlinks_pidfile_after_kill(self):
        ...
        with tempfile.TemporaryDirectory() as state:
            ...
            with mock.patch("translate.os.kill", side_effect=record_kill), \
...
class TestCLI(unittest.TestCase):
    ...
    def test_translate_once_english_defaults_to_spanish(self):
        out = io.StringIO()
        with mock.patch("translate.detect_language", return_value="en"), \
                mock.patch("translate.translate", return_value="Hola") as tr, \
                mock.patch("translate.speak"):
            translate_once(
                "Hello!", "http://x", None, None, True, stdout=out
            )
    ...
    def test_translate_once_explicit_from_skips_detection(self):
        out = io.StringIO()
        with mock.patch("translate.detect_language") as det, \
                mock.patch("translate.translate", return_value="Hello") as tr, \
                mock.patch("translate.speak"):
            translate_once(
    ...
    def test_translate_once_blank_input_noop(self):
        out = io.StringIO()
        with mock.patch("translate.translate") as tr:
    ...
    def test_run_repl_translates_lines_until_quit(self):
        ...
        with mock.patch("translate.detect_language", return_value="en"), \
                mock.patch("translate.translate", return_value="Hola"), \
                mock.patch("translate.speak", return_value=True), \
    ...
    def test_run_repl_survives_translate_error(self):
        out = io.StringIO()
        with mock.patch("translate.detect_language", return_value="en"), \
                mock.patch("translate.translate",
    ...
    def test_main_stop_server(self):
        with mock.patch("translate.stop_server", return_value=True) as stop, \
    ...
    def test_main_stop_server_rejects_extra_args(self):
        with mock.patch("translate.stop_server") as stop, \
                mock.patch("sys.stderr"):
    ...
    def test_main_one_shot_with_from_to(self):
        with mock.patch("translate.find_router_model", return_value=None), \
                mock.patch("translate.ensure_server", return_value="http://x"), \
                mock.patch("translate.detect_language") as det, \
                mock.patch("translate.translate", return_value="Привет"), \
                mock.patch("translate.speak", return_value=True), \
                mock.patch("sys.stdout"):
