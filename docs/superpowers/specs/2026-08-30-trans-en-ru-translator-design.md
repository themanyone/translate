...
A command-line app: give it an English phrase, it translates to Russian (or vice versa), prints the translation, and speaks it aloud with TTS. Language direction is detected automatically.
...
- **Piper TTS** at `/home/k/.local/sbin/piper` with `ru_RU-irina-medium` and `en_US-libritts_r-medium` voices (both 22050 Hz, `--cuda`).
...
4. Both piper voices produce WAV on stdout; `pw-play` plays it.

## Architecture

Single-file stdlib Python app (no pip dependencies): `trans` installed to `~/.local/bin/trans`, source lives in this repo.
...
2. **Language detection + direction**
   - Input contains any Cyrillic char (U+0400–U+04FF) → ru→en; else en→ru. Instant, deterministic.
   - (Both languages use disjoint scripts, so the heuristic is exact for this pair.)
...
4. **Speaker**
   - `piper --cuda --model <voice> -f -` (WAV to stdout) piped to `pw-play -` (fallbacks: `paplay`, `aplay`).
   - Voice chosen by output language: Russian → `ru_RU-irina-medium`, English → `en_US-libritts_r-medium`.
